...
import { Sells , Banks } from './../../../@interfaces';
import { HttpRequestService } from './../../../@services';

@Component({
    selector : 'sells-list',
    templateUrl : './sellsList.html',
    providers : [
        GlobalProvider,
        LabProvider,
        HttpRequestService,
...
export class SellsListComponent implements OnInit{
    ...
    private _controller:string = 'sells';
    private errors:any = {};
    // private formObject:FormGroup;
    private _list:Sells[];
    private _showItem:Sells;
    ...
    onAction($event):void{
        switch($event.action){
            case 'DELETE':
                ...
                break;
            case 'EDIT':
                let __url = 'sells/details/'+ $event.item.id.toString();
    ...
    __OnChangeStatus__(status:string):void{
        ...
        if(this.__status === 'RECEIPT' || this.__status === 'INVOICE'){
            status = 'completed';
            __additional = '&is_sell=' + new String(this.__status === 'RECEIPT');
            __additionalStr = '-is_sell';
    ...
    onAddNewClick($event):void{
        this._global.navigatePanel('sells/details');
    }
        ...
        this._http.get(__url).subscribe(
            (list)=>{
                ...
                if(this.__count >= 1){
                    ...
                    this._showItem = <Sells>new Object();
