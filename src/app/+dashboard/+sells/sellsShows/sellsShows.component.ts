...
import { Sells , Orders , Accounts } from './../../../@interfaces';
...
@Component({
    selector : 'sells-shows',
    templateUrl : './sellsShows.html',
    providers : [
        GlobalProvider,
        LabProvider,
        HttpRequestService,
...
export class SellsShowsComponent implements OnInit, OnChanges{
    ...
    OnDelete():void{
        ...
        this._http.delete('sells/' + this.item.id).subscribe(
            (item) => {
                if(item.data && !item.error){
                    if(!this.isDynamic) this._global.navigatePanel('sells');
                    return;
                }
    ...
    OnChangeStatus(status:string):void{
        if(!this.item || !this.item.id) return;
        this._http.put('sells/status/' + this.item.id , { status : status}).subscribe(
            (item) => {
                if(item.data && !item.error){
                    this.item = item.data;
                }else{
                ...
                (item) => {
                    ...
                    if(item.data.status === 'parked'){
                        if(!this.isDynamic){
                            ...
                            return this._global.navigatePanel('sells');
                        }
                    }
                    this.item = item.data;
                    this.__getOrders__();
