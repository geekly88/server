...
@Component({
    selector : 'collections-details',
    templateUrl : './collectionsDetails.html',
    providers : [
        GlobalProvider, 
        LabProvider, 
        HttpRequestService,
...
export class CollectionsDetailsComponent implements OnInit,AfterViewInit,OnChanges{
    
    public onAction = new EventEmitter();
    public isDynamic:boolean;
    public editable:boolean;
    public param:string;
    public editItem:Collections;
    private item:Collections;
    private submitted:boolean;
    private formObject:FormGroup;
    private errors:any;
    // private __obj:any;
    private _page:string = 'collections/';
    ...
    OnSubmitForm(values:Collections,valid:boolean):void{
        this.submitted = true;
        if(false === valid) {
            ...
            (item) => 
            {
                ...
                if(true === this.editable){
                    ...
                    for(let i=0; i < this.__collectionsArray.length; i++){
                        if(this.item.id === this.__collectionsArray[i].id){
                            this.__collectionsArray[i] = item.data;
                            break;
                        }
                    }
                }else{
                    ...
                    this.__collectionsArray.push(item.data);
                    this.__initFormsObject__();
                }
    ...
    public onSelectItem($event:any):void{
        if(!$event || !$event.target || !$event.target.name || $event.target.name !== 'father') return;
        for(let i=0; i < this.__collectionsArray.length; i++){
            if(this.__collectionsArray[i].name === $event.target.value){
                this.formObject.controls['father_id']['setValue'](this.__collectionsArray[i].id);
                break;
            }
        }
    }
    ...
    private __getCollections__():void{
        this._http.get('collections').subscribe(
            (items) => {
