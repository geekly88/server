...
export class BanksListComponent implements OnInit{
    ...
    private _controller:string = 'banks';
    private formObject:FormGroup;
    private errors:any;
    private _list:Banks[];
    private _showItem:Banks;
    ...
    onAction($event):void{
        switch($event.action){
            ...
            case 'SHOW':
                ...
                this._showItem = <Banks>$event.item;
    ...
    onUploadImage(item:Banks):void{
        ...
        myUploadItem.url = this._global.config['serverPath'] + 'banks/'+item.id+'/avatar?replace=true&token='+token;
    ...
    onAddNewClick($event):void{
        ...
        this.editItem = null;
        this._lab.__modal('#show-details-model');
        // this._global.navigatePanel('banks/details');
    }

    private __setObject():void {
       this._searchObjs = [
            { key : 'name' , value : 'اسم البنك' },
            { key : 'number' , value : 'رقم الحساب' },
        ...
        this._http.get(__url).subscribe(
            (list)=>{
                ...
                if(this.__count >= 1){
                    ...
                    this._showItem = <Banks>new Object();
    ...
    private __deleteItems(ids : string):void{
        ...
        this._http.delete(__url).subscribe(
            (response) => {
                ...
                this._lab.__setAlerts__('success' , 'تمت عملية حذف المورد ' + response.data.name);
            },
            (error) => {
                console.log(error);
                this._lab.__setErrors__(error);
