...
import { Sales , Banks } from './../../../@interfaces';
...
export class SalesListComponent implements OnInit{
    ...
    private _controller:string = 'sales';
    private errors:any = {};
    // private formObject:FormGroup;
    private _list:Sales[];
    private _showItem:Sales;
    ...
    onAction($event):void{
        switch($event.action){
            case 'DELETE':
                ...
                break;
            case 'EDIT':
                let __url = 'sales/details/'+ $event.item.id.toString();
    ...
    __OnChangeStatus__(status:string):void{
        ...
        let __additional:string= '';
        let __additionalStr:string= '';
        if(this.__order) __orderUrl = '&sortby=' + this.__order + '&sort=' + this.__sort;
        let __url = this._controller + '/?page=1&limit=' + this._global.getToken()['settings']['perpage'] + __orderUrl;
        if(this.__status === 'RECEIPT' || this.__status === 'INVOICE'){
            status = 'completed';
            __additional = '&is_sale=' + new String(this.__status === 'RECEIPT');
            __additionalStr = '-is_sale';
        ...
        __url += '&search=status'+__additionalStr+'&status='+ status + __additional;
    ...
    onAddNewClick($event):void{
        ...
        if(!__branches || !__branches.length){
            this._lab.__setAlerts__('warn' , 'تم تحويلك لصفحة تسجيل الدخول لوجود خطأ');
            return this._lab.__setLogout__(this._http);
        }
        if(__branches.length > 1) return this._lab.__setAlerts__('warn' , 'عليك مزامنة الفرع المسؤؤل عن عملية البيع فقط');
        this._global.navigatePanel('sales/details');
    }
        ...
        this._http.get(__url).subscribe(
            (list)=>{
                ...
                if(this.__count >= 1){
                    ...
                    this._showItem = <Sales>new Object();
