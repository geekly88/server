...
import { Sales, Branches, Storages, Customers, Types,Employees, Gifts , Orders, Products, Brands , Collections, Banks , RegistersHistory } from './../../../@interfaces';
...
export class SalesDetailsComponent implements OnInit,AfterViewInit{
    ...
    private __printSale__:boolean = false;
    private __showSaleData:any = {};
    private __param:string;
    private __orderNumber:number = 1;
    private __type__:string = 'completed';
    private __lastType__:string = 'completed';
    ...
    private _modal:string = 'sales';
    private __saleDate:Date = new Date();
    ...
    private __newSaleNumber:number = -1;
    private __parkSales:Sales[] = [];
    private __foundParkSales__:string = 'NOACTION';
    ...
    private __isSale__:boolean = false;
    ...
    private __quantityOrigin__:string = 'store';
    private __lastFocusedForm:FormGroup;
    private __lastFocusedIndex:number = -1;
    private __quantity:string = '';
    ...
    ngAfterViewInit() {
        ...
        let __saleType__:string = this._global.getToken()['settings']['sale_type'];
        if(__saleType__ === 'invoice' || __saleType__ === 'receipt'){
            this.__setSaleType__((__saleType__ === 'receipt'));
        }
        this._lab.prepareDiscountPopover();
        this._lab.__setOrdersShortCuts__(this,"Sale");
    }
    ...
    onFocusOrder(form:FormGroup , __id:number){
        ...
        let _self:SalesDetailsComponent = this;
    ...
    onParkOrders():void{
        ...
        if(false === this.formObject.valid) {
            if(this.formObject.controls['number'].valid !== true)
                if(this.__type__ === 'returned')
                    ...
                    // if(this.__newSaleNumber > 0 && this.__type__ === ''){
                        // this.formObject.controls['number'].setValue(this.__newSaleNumber);
                    // }else{
                        this.__createNewDetails__();
                    // }
        }
        ...
        this.onCompletePayment('',true, (values) => {
            ...
            this.__parkSales.push(Object['assign']({} , values));
            this._lab.jQuery('#show-sale-details-modal button.close').click();
            this._lab.jQuery('input.form-control.order-search.autofocus').focus();
        });        
    }
    ...
    OnSubmitForm(values:Sales,valid:boolean):void{
        this.submitted = true;
        if(false === valid) {
            if(this.formObject.controls['number'].valid !== true)
                if(this.__type__ === 'completed'){
                    if(this.__newSaleNumber){
                        this.formObject.controls['number'].setValue(this.__newSaleNumber);
                    }
                }else            
        ...
        + this.formObject.value.number + '&status=' + __status + '&register_id='+this.formObject.value['register_id']).subscribe(
            (item) => 
            {
                ...
                //             this.formObject.controls['number'].setErrors({'rule' : 'sale number is already exists'});
                //             return;
                //         }
                //     }
                // }else{
    ...
    __adjustDates__(sale:Sales = null):void{
        if(typeof(this.formObject.value.date) === 'string'){
            this.__saleDate = new Date(this.formObject.value.date);
            this.formObject.controls['date'].setValue(this.__saleDate);
            ...
            if(!this.formObject.value.date ||  isNaN(this.formObject.value.date.getDay())) {
                ...
                if(this._lastSale && this._lastSale.id === this.formObject.value.id && typeof this._lastSale.date === 'string'){ 
                    __date = new Date(<any>this._lastSale.date);
                }
        ...
        if(typeof(this.formObject.value.paid_date) === 'string'){
            ...
            if(!this.formObject.value.paid_date ||  isNaN(this.formObject.value.paid_date.getDay())) {
                let __date:Date = new Date();
                if(this._lastSale && this._lastSale.id === this.formObject.value.id && typeof this._lastSale.paid_date === 'string'){ 
                    __date = new Date(<any>this._lastSale.paid_date);
                }
                this.__paidDate = __date;
    ...
    OnPayClick():void{
        ...
        this.__refreshPrices();
        if(this.__type__ === 'primary'){
            return this.onCompletePayment('primary');
        }
            ...
            (items) => {
                ...
                if(!this._editable || this.__isParkEdit) {
                    ...
                    if(this._global.getToken()['settings']['is_show_invoice']){
                        ...
                        let self:SalesDetailsComponent = this;
                        this.__showSaleData = {
                            item : items.data,
                            orders : __orders,
                    ...
                    if(this.__continues__())
                        ...
                        if(this._lastSale.status === 'parked' && __didPayOut){
                    ...
                    this._lastSale = items.data;
                }
            },
            (error) => {
                this._lab.__setAlerts__(error);
                if(this.__type__ === 'primary') return;
    ...
    private __payAmountMethod__(type:string = ''):boolean{
        if(this.__type__ === 'primary') return true;
        //=================== PAYMENT ===============================\\
        //=================== PAYMENT ===============================\\
        //=================== PAYMENT ===============================\\
        //=================== PAYMENT ===============================\\
    ...
    private __init__():void{
        ...
        if(this.__branches.length > 1){
            this._lab.__setAlerts__('warn' , 'عليك مزامنة الفرع المسؤؤل عن عملية البيع فقط');
            this._global.navigatePanel(this._page.replace('/' , ''));
        }

        this.__storagesArray = <Storages[]>this._global.getResource('storages') || [];
        
        this.__quantityOrigin__ = <string>this._global.getResource('quantity_origin');
        if(!this.__quantityOrigin__) this.__quantityOrigin__ = 'store';
        let __quantity_origin_id:number = <number>this._global.getResource('quantity_origin_id');
        if(isNaN(__quantity_origin_id) || __quantity_origin_id <= 0 
        || this.__quantityOrigin__ === 'store' || !this.__storagesArray.length){
            this.__quantityOrigin__ = 'store';
            this._global.setResource('store' , 'quantity_origin');
            this._global.setResource( 0 , 'quantity_origin_id');
        }else{
            let __found:boolean = false;
            for(let i:number = 0; i < this.__storagesArray.length; i++){
                if(this.__storagesArray[i].id === __quantity_origin_id){
                    this.__quantityOrigin__ = 'storage';
                    this._global.setResource('storage' ,'quantity_origin');
                    this._global.setResource(__quantity_origin_id ,'quantity_origin_id');
                    __found = !__found;
                    break;
                }
            }
        ...
        //     }
            ...
            this.__checkIsSale__();
            
            // this.__checkIsStorage__();
            this.__getRouterParamsData__();
        // });
    }

    private __setType__(__type:string = null):void{
        if(__type === this.__type__) return;
        this.__lastType__ = this.__type__ === 'primary' ? this.__lastType__ : this.__type__ ;
        this.__type__ = __type;
    }

    private __initFormObject__():void{
        this.__textArea = this._global.getToken()['settings']['sale_terms'] || '';
        this.formObject = this._fb.group({
            ...
            is_sale            : [ this.__isSale__ ],
            // is_stock_from_storage: [ this.__isStorage__ ],
            ...
            quantity_origin    : [ this.__quantityOrigin__ , [Validators.required , Validators.pattern(this.nameRegex)]],
            quantity_origin_id : [ this._global.getResource('quantity_origin_id') , [Validators.required , Validators.pattern(this.intRegex)]],
    ...
    private __getRouterParamsData__(sale:Sales = null):void{
        ...
        if(sale && sale.id && sale.id > 0){
            this._editable = true;
            this.__param = sale.id.toString();
            this.__isParkEdit = true;
            for(let key in sale){
                if(sale[key] !== null && this.formObject.value.hasOwnProperty(key)){
                    this.formObject.controls[key].setValue(sale[key]);
                }
            }
            ...
            if(sale.orders && sale.orders.length > 0){
                this.__orders = [];
                (<Array<Orders>>sale.orders).forEach((order,index)=>{
            ...
            this.__showModalDetails__('sales');
            this._lab.jQuery('input.form-control.order-search.autofocus').focus();
            this.__paid_arr_backup = this.formObject.value['payments'];
            setTimeout(() => { this.__adjustDates__(sale) } , 1000);
            ...
            this._http.get(this._page + this.__param).subscribe(
                (item) => {
                    ...
                    this.__textArea = this.formObject.value['description'];
                    this._lastSale = <Sales>item.data;
                    if(this.formObject.value.status === 'parked' && this.formObject.value.parked_msg && this._lastSale.parked_msg !== ''){
                        this.__hasParkNote = true;
                        this.__showModalDetails__('parkMsg');
                    }
                    if(this.__type__ !== 'primary'){
        ...
        if(!sale || !sale.id) this.__initRegister__();
    }
    ...
    __checkParkSales__():void{
        if(this.__foundParkSales__ !== 'NODATA'){
            this.__getMoreParkSales(true);
        }
        this.__showModalDetails__('parksales');
    }

    __getMoreParkSales(first:boolean = false):void{
        if(!this.__registerOpen || !this.__register) return;
        // if(this.__parkSales.length < this._global.getToken['settings']['perpage']) return;
        let __page:number = first ? 1 : (Math.ceil(this.__parkSales.length / this._global.getToken()['settings']['perpage']) + 1);
        let __URL__:string = 'sales/?sortby=id&sort=DESC&search=status-register_id&status=parked';
        __URL__ += '&register_id='+this.__register.register_id+'&limit='+this._global.getToken()['settings']['perpage'];
        __URL__ += '&page=' + (Math.ceil(this.__parkSales.length / this._global.getToken()['settings']['perpage']) + 1);
        this._http.get(__URL__).subscribe(
            (items) => {
                if(items.error){
                    ...
                    this.__foundParkSales__ = 'HASDATA';
                        for(let i=0; i < items.data.length; i++){
                            this.__parkSales.push(items.data[i]);
                        }
                }else{
                    if(items.data.length === 0){
                        return this._lab.__setAlerts__('warn' , 'لايوجد طلبات أخرى لاظهارها');
                    }
                    this.__foundParkSales__ = 'NODATA';
                }
            },(errorr)=>{
    ...
    __onParkedSaleAction(sale:Sales , action:string):void{
        switch (action) {
            case 'DELETE':
                if(sale.id > 0) return;
                this._http.delete('sales/' + sale.id).subscribe(
                    (item) => {
                        if(item.error){
                            return this._lab.__setErrors__(item.error);
                        }
                        if(item.data){
                            this.__removeParkSaleFromArray__(sale);
                        }
                    }
                );
                break;
            case 'EDIT':
                this.__clearProperties__();
                this.__getRouterParamsData__(sale);
                break;
        }
    }

    private __removeParkSaleFromArray__(sale:Sales):void{
        let __sales:Array<Sales> = this.__parkSales;
        this.__parkSales = [];
        __sales.forEach((__sale , index) => {
            if(__sale.id !== sale.id){
                this.__parkSales.push(__sale);
            }
        });
    }
    ...
    private __initOrderInformations__():void{
        ...
        this._http.get(this._page + '?limit=1&page=1').subscribe(
            (item) => {
                ...
                this._lastSale = <Sales>item.data[0];
                if(this._lastSale.status === 'primary' || this._lastSale.status === 'parked'){
                    for(let key in this._lastSale){
                        if(this._lastSale[key] !== null && this._lastSale[key] !== undefined && this.formObject.value.hasOwnProperty(key)){
                            this.formObject.controls[key].setValue(this._lastSale[key]);
                        }
                    }
    ...
    private __checkIsSale__():void{
        if(this.__param && this._editable) return;
        this.__isSale__ = Boolean(this._global.getResource('is_sale'));
        this.formObject.controls['is_sale'].setValue(this.__isSale__);
    }

    // private __checkIsStorage__():void{
    //     if(this.__param && this._editable) return;
    //     this.__isStorage__ = Boolean(this._global.getResource('is_stock_from_storage'));
    //     this.formObject.controls['is_stock_from_storage'].setValue(this.__isStorage__);
    // }

    private __setSaleType__(is_sale:boolean = true):void{
        if(this.__param && this._editable) return;
        this.__isSale__ = is_sale;
        this.formObject.controls['is_sale'].setValue(this.__isSale__);
        this._global.setResource(this.__isSale__ , 'is_sale');
        this.__createNewDetails__();
    }


    // private __setStockType__(is_stock_from_storage:boolean = true):void{
    //     this.__isStorage__ = is_stock_from_storage;
    //     this.formObject.controls['is_stock_from_storage'].setValue(this.__isStorage__);
    //     this._global.setResource(this.__isStorage__ , 'is_stock_from_storage');
    // }
    ...
    __retriveOrders__(__continue:boolean = false):void{
        ...
        this._http.get(__url).subscribe(
            (items) => {
                ...
                && (this.formObject.value.status === 'parked' || this.formObject.value.status === 'primary')){
    ...
    __showModalDetails__(modal:string = 'sales' , __opt = {}){
        this._modal = modal;
        this._lab.__modal('#show-sale-details-modal', __opt);
    ...
    private __createOrderFormGroup__(item:any , qty:number = null):FormGroup{
        ...
        if(item.hasOwnProperty('limit_sale_quantity')){
            __is_product = true;
        ...
        let __quantity_origin:string = item.hasOwnProperty('quantity_origin') ? item.quantity_origin : this.formObject.value.quantity_origin;
        let __quantity_origin_id:number = item.hasOwnProperty('quantity_origin_id') ? item.quantity_origin_id : this.formObject.value.quantity_origin_id;
        __quantity_origin_id = __quantity_origin === 'store' ? 0 : __quantity_origin_id;
        let __order:FormGroup = this._fb.group({
            ...
            quantity_origin   : [ __quantity_origin ],
            quantity_origin_id : [ __quantity_origin_id ],
    ...
    __setQuantityOriginStorage__($event):void{
        console.log($event);
        this.__setQuantityOrigin__('storage', $event);
    }

    __setQuantityOrigin__(type:string = null,$event:any=null):void{
        ...
        this.__quantityOrigin__ = type;
        this.formObject.controls['quantity_origin'].setValue(type);
        this.formObject.controls['quantity_origin_id'].setValue(id);
        this._global.setResource(type, 'quantity_origin');
        this._global.setResource(id  , 'quantity_origin_id');
    }

    __setQuantityOriginForm__(form:FormGroup , $event):void{
        if(!form) return;
        let id = parseInt($event.target.value);
        if(isNaN(id)) id = this.formObject.value['quantity_origin_id'];
        if(id === 0) form.controls['quantity_origin'].setValue('store');
        else form.controls['quantity_origin'].setValue('storage');
        form.controls['quantity_origin_id'].setValue(id);
    }
    ...
    __clearProperties__(__back:boolean = true){
        if(!this._editable || this.__isParkEdit){
            ...
            this._lastSale = <Sales>this.formObject.value;
            ...
            if(this.__isParkEdit){
                ...
                this.__isParkEdit = false;
                this.__hasParkNote = false;
                this.__removeParkSaleFromArray__(this._lastSale);
            }
            // this.formObject.controls['register_closure'].setValue(this.__register.closure);
            // this.formObject.controls['register'].setValue(this.__register.register);
            // this.formObject.controls['register_id'].setValue(this.__register.register_id);
    ...
    private __continues__(is_msg:boolean = true):boolean{
        // if(this._editable && this.__param && !this.__isParkEdit && this._lastSale.status !== 'parked'){
        //     if(is_msg)
    ...
    OnShowQuantityModal(form:FormGroup,index:number):void{
        if(!form || isNaN(index) || index < 0) return;
        this.__lastFocusedIndex = index;
        this.__lastFocusedForm = form;
        this.__quantity = '';
        this.__showModalDetails__('quantity');
    }

    OnUpdateQuantity(__btnClicked:string):void{
        if(!__btnClicked) return;
        switch (__btnClicked) {
            case '0':
            case '00':
                if(!this.__quantity) this.__quantity = '0';
                else this.__quantity += __btnClicked;
                break;
            case 'dot':
                if(this.__quantity.indexOf('.') > 0) return;
                if(!this.__quantity || this.__quantity === '0'){
                    this.__quantity += '0.';
                }else{
                    this.__quantity += '.';
                ...
                break;
            case 'backspace':
                if(!this.__quantity || this.__quantity.length === 0) return;
                this.__quantity = this.__quantity.substr(1 , this.__quantity.length - 1);
                break;
            case 'clear':
            case 'cancel':
                this.__quantity = '';
            case 'confirm':
                if(!this.__quantity || this.__quantity === '0') return;
                this.__orders[this.__lastFocusedIndex].controls['quantity'].setValue(parseFloat(this.__quantity));
            default:
                if(this.__quantity === '0') this.__quantity = '';
                this.__quantity += __btnClicked;
                break;
        }
    }
}