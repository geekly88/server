...
export class CustomersDetailsComponent implements OnInit,AfterViewInit{
    ...
    private item:Customers;
    private errors:any;
    private _page:string = 'customers/';
    ...
    OnSubmitForm(values:Customers,valid:boolean):void{
        this.submitted = true;
        if(false === valid) {
        ...
        obj.subscribe(
            (item) => 
            {
                ...
                if(true === this._editable){
                    ...
                    // if(__booksTree){
                    //     let __code:string = this._global.getToken()['settings']['customers_code'] + this.item.code;
                    //     for(let i=0; i < __booksTree.length; i++){
                    //         if(__booksTree[i].code === __code){
                    //             __booksTree[i].code = this._global.getToken()['settings']['customers_code'] + item.data.code;
                    ...
                    let __item:Customers = item.data;
    ...
    private __getRouterParamsData__():void{
        ...
        if(!this.isDynamic && this._route.params['value'] && this._route.params['value']['id'] &&
            ...
            this._http.get(this._page + this.__param).subscribe(
                (item) => {
                    if(item.error !== null) { 
                        this._router.navigate([this._global.config['dashboard'] + '/customers']);
                        return;
                    }
                    this.item = item.data;
                    for(let key in this.item){
