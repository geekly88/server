...
@Component({
    selector : 'storages-list',
    templateUrl : './storagesList.html',
    providers : [
        GlobalProvider, 
        LabProvider, 
        HttpRequestService,
...
export class StoragesListComponent implements OnInit{
    ...
    private _controller:string = 'storages';
    private formObject:FormGroup;
    private errors:any;
    private _list:Storages[];
    private _showItem:Storages;
    ...
    onAction($event):void{
        switch($event.action){
            ...
            case 'SHOW':
                ...
                this._showItem = <Storages>$event.item;
        ...
        this._http.get(__url).subscribe(
            (list)=>{
                ...
                if(this.__count >= 1){
                    ...
                    this._showItem = <Storages>new Object();
    ...
    private __deleteItems(ids : string):void{
        ...
        this._http.delete(__url).subscribe(
            (response) => {
                ...
                this._lab.__setAlerts__('success' , 'تمت عملية حذف المخزن ' + response.data.name);
            },
            (error) => {
                this._lab.__setErrors__(error);
            },
