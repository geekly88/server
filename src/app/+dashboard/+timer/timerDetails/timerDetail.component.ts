...
@Component({
    selector : 'timer-details',
    templateUrl : './timerDetails.html',
    providers : [
        GlobalProvider, 
        LabProvider, 
        HttpRequestService,
...
export class TimerDetailsComponent implements OnInit,AfterViewInit,OnChanges{
    
    public onAction = new EventEmitter();
    public isDynamic:boolean;
    public editable:boolean;
    public param:string;
    public editItem:Timer;
    private item:Timer;
    private submitted:boolean;
    private formObject:FormGroup;
    private errors:any;
    private __obj:any;
    private _page:string = 'timer/';
    constructor(
    ...
    OnSubmitForm(values:Timer,valid:boolean):void{
        this.submitted = true;
        if(false === valid) {
    ...
    private __initFormsObject__(time:number = 0):void{
        ...
        this.formObject = this._fb.group({
            ...
            is_done : [ true ],
            note    : [ '' , Validators.pattern(nameRegex)],
        });
    }
