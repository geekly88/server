...
import { Purchases , Banks } from './../../../@interfaces';
import { HttpRequestService } from './../../../@services';

@Component({
    selector : 'purchases-list',
    templateUrl : './purchasesList.html',
    providers : [
        GlobalProvider,
        LabProvider,
        HttpRequestService,
...
export class PurchasesListComponent implements OnInit{
    ...
    private _controller:string = 'purchases';
    private formObject:FormGroup;
    private errors:any;
    private _list:Purchases[];
    private _showItem:Purchases;
    ...
    private __paymentItem:Purchases;
    ...
    onAction($event):void{
        switch($event.action){
            case 'DELETE':
                ...
                break;
            case 'EDIT':
                let __url = 'purchases/details/'+ $event.item.id.toString();
    ...
    onAddNewClick($event):void{
        ...
        if(__branches.length > 1) return this._lab.__setAlerts__('warn' , 'عليك مزامنة الفرع المسؤؤل عن عملية الشراء فقط');
        this._global.navigatePanel('purchases/details');
    }
        ...
        this._http.get(__url).subscribe(
            (list)=>{
                ...
                if(this.__count >= 1){
                    ...
                    this._showItem = <Purchases>new Object();
