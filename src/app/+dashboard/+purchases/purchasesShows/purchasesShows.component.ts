...
import { Purchases , PurchaseOrders , Accounts } from './../../../@interfaces';
...
@Component({
    selector : 'purchases-shows',
    templateUrl : './purchasesShows.html',
    providers : [
        GlobalProvider, 
        LabProvider, 
        HttpRequestService,
...
export class PurchasesShowsComponent implements OnInit, OnChanges{
    ...
    ngOnInit():any{
        this.__init__();
        this.__perpage = this._global.config['purchases_report_perpage'];
    ...
    ngOnChanges(props:any){
        ...
        if(props.data && props.data.currentValue){
            ...
            this.purchaseOrders = props.data.currentValue.purchaseOrders;
            this.__preparePagesAndPurchaseOrders();
        }
    ...
    OnDelete():void{
        ...
        this._http.delete('purchases/' + this.item.id).subscribe(
            (item) => {
                if(item.data && !item.error){
                    return this._global.navigatePanel('purchases');
                }
    ...
    OnChangeStatus(status:string):void{
        if(!this.item || !this.item.id) return;
        this._http.put('purchases/status/' + this.item.id , { status : status}).subscribe(
            (item) => {
                if(item.data && !item.error){
                    this.item = item.data;
                }else{
    ...
    private __preparePagesAndPurchaseOrders():void{
        this.__initPrintFormObject__();
        this.__pagesShown = [];
        this.__pagesCount = 0;
        this.__pages = [];
        if(!this.purchaseOrders || !this.purchaseOrders.hasOwnProperty('length') || !this.purchaseOrders.length){
            this.purchaseOrders = [];
        }
        this.__pagesCount = Math.ceil(this.purchaseOrders.length / this.__perpage);
        ...
        for(let i = 0; i < this.purchaseOrders.length; i++){
            ...
            if(!this.__pages[__number]){
                this.__pages[__number] = [this.purchaseOrders[i]];
                this.__pagesShown.push(__number === 0);
            }else{
                this.__pages[__number].push(this.purchaseOrders[i]);
            }
        }
    }
    ...
    private __getPurchaseOrders__():void{
        this._http.get('purchaseOrders/?search=father_id&father_id=' + this.item.id).subscribe(
            (items) => {
                ...
                this.purchaseOrders = <PurchaseOrders[]>items.data;
                this.__preparePagesAndPurchaseOrders();
